...
- 마지막 갱신: 2026-08-09 00:15 KST
- 기준 저장소: `C:\GameDev\GameProject\project-nhn`
- 기준 원격 브랜치: `origin/main`
- 확인된 기준 커밋: `de05220` (`픽토그램 UI 배포 상태 기록 병합`)
...
|---|---|---|---|
| 타이틀 화면 | 로컬 구현·Web 검증 완료 | 그래픽 디자이너용 빈 일러스트 슬롯과 하단 중앙 `게임 시작`·`옵션`만 표시하는 시작 장면을 추가. 타이틀 옵션은 안내·돌아가기만 제공하고 테스트 진입을 노출하지 않으며, 명시적 자동 검증 인자만 타이틀을 우회함. 전용 검사·기존 테스트 환경 검사·Web Release 및 로컬 Web 초기/옵션/복원/일반 시작 화면과 콘솔 확인 완료 | 타이틀 일러스트 수령 후 `IllustrationSlot`에 연결하고 공개 배포 확인 |
...
| Pages 배포 | 완료 | merge commit `54b45a9`의 실행 `31257341031` 성공. 공개 HTML의 `index-54b45a92bdbb.pck` 참조, 16,700,800-byte PCK와 로컬·원격 SHA-256 `35F4688B87A9939987A2868BD3B6BC9F14DF4E362E3048D85755EBF47DE1D97D` 일치 확인 | 공개 플레이에서 픽토그램 크기·2배속 간격과 기존 전투 기능 체감 확인 |

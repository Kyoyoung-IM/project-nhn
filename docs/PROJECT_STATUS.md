...
|---|---|---|---|
| 게임 크레딧 | 로컬 구현·Web 검증 완료 | 타이틀 하단에 `게임 크레딧`을 추가하고 사용자 제공·OpenAI 생성·Jua·Godot 출처와 라이선스를 대형 스크롤 패널에 표시. 미확인 사용자 제공 리소스는 원출처·배포 라이선스 미제공과 팀 확인 필요 상태를 명시함. 전용 검사·기존 테스트 환경 검사·Web Release 및 로컬 Web 초기/스크롤/복원 화면·콘솔 확인 완료 | 사용자 제공 리소스의 원출처와 배포 권한을 팀에서 확인 후 크레딧 문구 갱신 |
...
| Pages 배포 | 완료 | merge commit `346bdb1`의 실행 `31264059819` 성공. 공개 HTML의 `index-346bdb186265.pck` 참조, 16,711,728-byte PCK와 로컬·원격 SHA-256 `67C20A0FDE7B1BBF598BA346BADB2C4ACA23806711CA51D5E76C957B2EC44B07` 일치 확인 | 공개 플레이에서 타이틀 일러스트 적용 전 버튼 위치 체감 확인 |

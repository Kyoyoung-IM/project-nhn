...
| 설치 드롭 판정 | 로컬 `main` 병합 완료 | 상점 카드 설치와 설치 타워 이동·머지의 슬롯 중심 드롭 반경을 `70px`에서 `120px`로 확대. 슬롯 간격 `240px`의 절반으로 제한해 이웃 판정 중첩을 방지하고 전용 검사·Web 경계 안 설치·이동·콘솔 확인 완료 | 원격 반영 후 공개 플레이에서 확대된 드롭 조작감 체감 확인 |
...
| Pages 배포 | 완료 | merge commit `6156b32`의 실행 `31265045005` 성공. 공개 HTML의 `index-6156b329b285.pck` 참조, 16,719,656-byte PCK와 로컬·원격 SHA-256 `B7DBBFD6EF5DEB258CDF841DE077CF79F2935D32BA08098383D8E4C254B70AA7` 일치 확인 | 사용자 제공 리소스 권리 확인 후 공개 크레딧 문구 갱신 |

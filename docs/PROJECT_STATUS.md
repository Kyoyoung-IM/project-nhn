...
- 마지막 갱신: 2026-08-08 18:18 KST
- 기준 저장소: `C:\GameDev\GameProject\project-nhn`
- 기준 원격 브랜치: `origin/main`
- 확인된 기준 커밋: `50d9596` (`Merge pull request #67 from Kyoyoung-IM/agent/github-message-reporting-20260808`)
...
| 프로젝트 관리 지침 | GitHub 인증 흐름 개선 반영 | `agent/github-auth-fallback-guideline-20260808`에서 Git 전송·GitHub 앱·CLI 인증을 분리하고, 앱 지원 작업은 앱 우선·CLI 인증은 필요한 순간에만 확인하도록 지침 수정 | 문서 검증 후 게시 여부 결정 |
| Pages 배포 | 완료 | PR #65 병합 SHA `8746ff5`의 실행 `31249805629` 성공. 공개 HTML의 `index-8746ff5f9973.pck` 참조, 16,685,760-byte PCK와 로컬·원격 SHA-256 `F1D5F20CA3F075774541290D2FE262292316C3F27A46534C6721805410E3E1B4` 일치 확인 | 다음 기능 병합 후 Web Release 갱신 여부 재확인 |
...
- Git 전송은 `git`, 지원되는 저장소·이슈·PR 서비스 작업은 GitHub 앱을 기본으로 사용한다. Actions 상세 로그 등 앱 미지원 작업에만 `C:\Program Files\GitHub CLI\gh.exe`를 사용한다.
- CLI 인증은 작업 시작이 아니라 CLI가 꼭 필요한 단계 직전에 한 번만 확인한다. CLI 인증 실패만으로 Git 전송이나 앱 지원 작업을 중단하지 않으며, CLI 전용 필수 단계가 실제로 막힐 때만 재로그인을 요청한다.
- GitHub 앱 쓰기를 시험 호출하지 않는다. 실제 승인 작업에서 앱 권한 오류가 발생하면 반복하지 않고 인증된 CLI로 한 번 전환하며, 두 경로가 모두 막힌 경우에만 사용자 조치를 요청한다.
